...
class model_component_menus_manager extends model {
	...
	*/
	...
	public function add_menu($data = array()) {
		$sql = "INSERT INTO menus (name, class, language)
							VALUES ('" . $this->dbh->escape($data['name']) . "', 
									'" . $this->dbh->escape($data['class']) . "', 
									'" . $this->dbh->escape($data['language']) . "')";
		$this->dbh->exec($sql);
	...
	public function edit_menu($data = array()) {
		$sql = "UPDATE menus SET 
					...
					language 	= '" . $this->dbh->escape($data['language']) . "'
