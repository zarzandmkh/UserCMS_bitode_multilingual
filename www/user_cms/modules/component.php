...
class component extends module {
	...
	//this function varies from original/ modified for multilingual edition
	public function load_view($name = 'index') {
		$full_name = $this->view_dir . '/' . $name . '.tpl';
		$full_name_core = $this->view_dir_core . '/' . $name . '.tpl';

		if(!empty(LANGUAGE_GLOBAL) && is_file($this->view_dir . '/language/' . LANGUAGE_GLOBAL . '/language.php')){
			include $this->view_dir . '/language/' . LANGUAGE_GLOBAL . '/language.php';
			$this->data = array_merge($this->data, $data_language);
		}
